...
def index_view(request):
    ...
    form = TodoForm()
    return render(request, 'index.html', {"name": request.user.username, "todos": all_todos, "user": request.user, "new_todo": form})
...
def done_view(request):
    if request.method == 'POST':
        all_todo = ToDo.objects.filter(user=request.user, id__in=request.POST.getlist('done'))
